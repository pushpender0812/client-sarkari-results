...
const SingleAdmission = () => {
    ...
    const getSingleAdmission = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/single-admission?id=${id}`, {
                method: 'GET',
                headers: {
  ...
  return (
    <div className="job-single-container">
            <div className="job-details">
                <h1 className="highlighted-title"><strong style={{color:'red'}}>Admission :</strong> {admission?.post_name}</h1>
                <p><strong style={{color:'red'}}>Post Date :</strong> {formatDate(admission?.createdAt)}</p>
                <p><strong style={{color:'red'}}>Update Date :</strong> {formatDate(admission?.updatedAt)}</p>
                <p><strong style={{color:'red'}}>Short Description of The Admission :</strong> {admission?.job_description}</p>
            </div>
            ...
            <div className="how-to-fill">
                ...
                <ul>
                    {splitContentToList(admission?.how_to_fill)}
                </ul>
            </div>
